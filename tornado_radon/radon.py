...
#koeri version of liviu's tornado app
...
(
select
      a.CountRn as radon_value,
      DATE_FORMAT(a.time_acq,'%Y-%m-%dT%H:%i:%s') as value_time,
      b.network as network_code,
      b.stacode as station_code,
      b.loco as station_location,
      b.lat as station_latitude,
      b.lon as station_longitude,
      b.elv as station_elevation
from
rn_count a,
stations b
#installation_type c,
...
# sens_t_int e
where
a.stacode = b.stacode and
a.network = b.network and
a.loco = b.loco and
(b.lat between {minlat} and {maxlat}) and
(b.lon between {minlon} and {maxlon}) and
(a.time_acq between '{mintime}' and '{maxtime}')
#abs(TIMESTAMPDIFF(microsecond,a.time_acq,e.time_acq)/{DIV}) < {max_int_delta}
#(b.samp_rate between {min_period} and {max_period}) and c.value like '{type_site}' having radon_error <= {max_radon_error}
);
select
radon_value,
#radon_error,
...
class IndexHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        queries.append(dict(
            ...
            minlon=28.0,
            maxlon=31.0,
            mintime='2012-11-01T00:00:00.000',
            maxtime='2012-11-02T00:00:00.000',
            # min_period='PT60S',
            # max_period='PT180S',
            #min_period='60',
            #max_period='180',
        ...
        queries = ['/nfo_marsite/radon/query?%s' % q for q in queries]
